...
HOME="$ALICE_HOME" USERPROFILE="$ALICE_HOME" DOTENVUP_TEST=1 DOTENVUP_IDENTITY_DIR="$ALICE_ID" DOTENVUP_TEST_IDENTITY_DIR="$ALICE_ID" \
  node "$CLI_BIN" init

echo "[qa] Generate Bob key"
HOME="$BOB_HOME" USERPROFILE="$BOB_HOME" DOTENVUP_TEST=1 DOTENVUP_IDENTITY_DIR="$BOB_ID" DOTENVUP_TEST_IDENTITY_DIR="$BOB_ID" \
  node "$CLI_BIN" init
